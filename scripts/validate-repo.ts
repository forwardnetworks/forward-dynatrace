...
const fail = (message: string): number => failures.push(message);
...
const discoverySchema = JSON.parse(
  await readText("settings/schemas/dependency-discovery-profile.schema.json"),
) as {
  schemaId?: string;
  version?: string;
  properties?: {
