...
import type { ForwardExportManifest } from "../lib/intent-builder.ts";
import type { DependencyCandidate } from "../lib/types/forward.ts";
import {
...
interface PackageArgs {
  ...
  "forward-base-url"?: string;
  "forward-network-id"?: string;
  "nqe-query-id"?: string;
  "nqe-diff-query-id"?: string;
  "nqe-diff-before-snapshot-id"?: string;
  "nqe-diff-after-snapshot-id"?: string;
  "output-dir"?: string;
  "eligibility-report"?: string;
  "include-review"?: true;
  "enable-performance-monitoring"?: true;
  "sync-mode"?: string;
  "forward-access-profile"?: string;
}

type ExtendedManifest = ForwardExportManifest & {
  artifacts: ForwardExportManifest["artifacts"] & {
    nqeChecks?: string;
    nqeDiffRequests?: string;
  };
  integrity: ForwardExportManifest["integrity"] & {
    nqeChecksSha256?: string;
    nqeDiffRequestsSha256?: string;
  };
  nqeChecks?: Record<string, unknown>;
  nqeDiffRequests?: Record<string, unknown>;
};
...
Options:
  ...
  --enable-performance-monitoring Enable Forward performance monitoring on generated intent checks.
  --sync-mode direct-api          Direct Forward API synchronization (default and only mode).
...
This offline preview does not contact Forward. Runtime reads and approved writes use the bundled Dynatrace action.
`;

const parseArgs = (argv: string[]): PackageArgs => {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (key === "help" || key === "include-review" || key === "enable-performance-monitoring") {
      args[key] = true;
      continue;
    }
...
const requiredDependencyFields: Array<keyof DependencyCandidate> = [
  "source",
  "destination",
  "protocol",
  "port",
...
const missingFields = (dependency: DependencyCandidate): string[] =>
...
const eligibilityReason = (
  ...
  return `Blocked: unsupported mappingState ${String(
    (dependency as { mappingState: unknown }).mappingState,
  )}.`;
};
...
const buildEligibilityReport = ({
  ...
  const count = (predicate: (row: (typeof rows)[number]) => boolean): number =>
...
const main = async (): Promise<void> => {
  ...
  const syncMode = args["sync-mode"] || "direct-api";
  ...
  const dependenciesValue: unknown = JSON.parse(await readFile(args.dependencies, "utf8"));
  if (!Array.isArray(dependenciesValue)) {
    throw new Error("--dependencies must contain a JSON array.");
  }
  const dependencies = dependenciesValue as DependencyCandidate[];
  ...
  const result = forwardSync({
    ...
    includeReviewRows,
    enablePerformanceMonitoring: Boolean(args["enable-performance-monitoring"]),
    dependencies,
  });
  ...
  const manifest = JSON.parse(result.exportManifestPreview) as ExtendedManifest;
  let nqeChecks: ReturnType<typeof buildNqeChecksFromDependencies> = [];
  let nqeDiffRequests: ReturnType<typeof buildNqeDiffRequestsFromDependencies> = [];
  ...
  if (
    ...
    validateNqeDiffRequests(nqeDiffRequests, {
      allowedQueryIds: new Set(
        args["nqe-diff-query-id"] ? [args["nqe-diff-query-id"]] : [],
      ),
    });
    ...
    manifest.nqeDiffRequests = {
      ...
      queryIdPolicy: "forward-owned-allowlist",
      executionPolicy: "read-only-app-backend-optional",
      parameterSource: "dynatrace-app-environment",
    };
  }
...
main().catch((error) => {
  process.stderr.write(`${error instanceof Error ? error.message : "Forward package build failed."}\n`);
