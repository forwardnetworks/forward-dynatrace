...
import { normalizeDynatraceRows } from "./normalize-dynatrace-dependencies.ts";
...
interface QueryArgs {
  ...
  "max-result-records"?: string;
  output?: string;
  "poll-interval-ms"?: string;
  "poll-timeout-ms"?: string;
  query?: string;
  "query-file"?: string;
...
interface DynatraceQueryResponse {
  state?: string;
  requestToken?: string;
...
const parseArgs = (argv: string[]): QueryArgs => {
...
const extractToken = (rawValue: string): string => {
...
const readToken = async (tokenFile: string | undefined): Promise<string> => {
...
const toPlatformApiBaseUrl = (environmentUrl: string): string => {
  const url = new URL(environmentUrl);
  return url.origin;
};

const toPositiveInteger = (value: string, label: string): number => {
...
const readQuery = async (args: QueryArgs): Promise<string> => {
...
const readResponseJson = async (
  response: Response,
  label: string,
): Promise<DynatraceQueryResponse> => {
...
const executeQuery = async ({
  ...
  return Array.isArray(result.result?.records) ? result.result.records : [];
};
...
const main = async (): Promise<void> => {
  ...
  const configuredEnvironmentUrl = isRecord(appConfig) &&
    typeof appConfig.environmentUrl === "string"
  ...
  if (!environmentUrl) {
    throw new Error("Missing Dynatrace environment URL.");
  }
  const apiBaseUrl =
    args["api-base-url"] ||
  ...
  let dependencies: ReturnType<typeof normalizeDynatraceRows> = [];
...
main().catch((error) => {
  const message = error instanceof Error ? error.message : "Dynatrace dependency query failed.";
