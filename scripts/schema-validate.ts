...
interface SchemaArgs {
  ...
  status?: string;
  "status-event"?: string;
  "network-evidence-event"?: string;
  "change-context"?: string;
  "change-validation-gate"?: string;
  "change-validation-event"?: string;
  "guardian-context"?: string;
  "check-health-transitions"?: string;
  "security-correlation"?: string;
  "security-correlation-event-batch"?: string;
}

type Validators = Record<SchemaName, ValidateFunction>;
...
const parseArgs = (argv: string[]): SchemaArgs => {
  const args: SchemaArgs = {};
...
const readJson = async (relativeOrAbsolutePath: string): Promise<unknown> =>
  JSON.parse(
    await readFile(
...
const buildValidator = async (): Promise<Validators> => {
  ...
  const schemas = Object.fromEntries(
    await Promise.all(
      Object.entries(schemaPaths).map(async ([name, schemaPath]) => [
        name,
        await readJson(schemaPath) as AnySchema,
      ]),
    ),
  ) as Record<SchemaName, AnySchema>;
  for (const schema of Object.values(schemas)) ajv.addSchema(schema);
  const validators = {} as Partial<Validators>;
  for (const [name, schema] of Object.entries(schemas) as Array<[SchemaName, AnySchema]>) {
    const schemaId = typeof schema === "object" && schema !== null && "$id" in schema
      ? String(schema.$id)
      : "";
    const validator = ajv.getSchema(schemaId);
    if (!validator) throw new Error(`Schema ${name} did not register a validator.`);
    validators[name] = validator;
  }
  return validators as Validators;
};

const failText = (validator: ValidateFunction): string =>
  (validator.errors || [])
...
const validatePackageDir = async (
...
const buildValidationArtifacts = async (): Promise<string> => {
