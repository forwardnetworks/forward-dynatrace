...
Options:
  ...
  --archive-output FILE   Copy the single dry-run .zip bundle to FILE.
...
export const parseArgs = (argv) => {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (valueOptions.has(arg)) {
      ...
      if (arg === "--archive-output") args.archiveOutput = value;
...
export const validateDeployArgs = (args, defaultAppId, env = process.env) => {
  ...
  if (args.archiveOutput && !args.dryRun) {
    throw new Error("--archive-output requires --dry-run.");
  }
  if (args.archiveOutput && args.uninstall) {
    throw new Error("--archive-output cannot be combined with --uninstall.");
  }
...
const findFilesBySuffix = async (directory, suffix) => {
  const matches = [];
...
const run = async () => {
  ...
  try {
    ...
    if (exitCode === 0 && args.archiveOutput) {
      const archives = await findFilesBySuffix(path.join(deployRoot, "out"), ".zip");
      if (archives.length !== 1) {
        throw new Error(
          `Expected exactly one Dynatrace dry-run archive; found ${archives.length}.`,
        );
      }
      const archiveOutput = path.resolve(root, args.archiveOutput);
      await copyFile(archives[0], archiveOutput);
      process.stdout.write(`Copied Dynatrace app archive to ${archiveOutput}\n`);
    }
