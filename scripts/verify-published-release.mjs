...
The verifier requires authenticated GitHub CLI read access to the release,
attestations, and release workflow. It performs no writes and emits
...
export const validateReleaseMetadata = (metadata, releaseName, repository = DEFAULT_REPOSITORY) => {
  ...
  if (metadata.isPrerelease) throw new Error("GitHub release is unexpectedly marked as a prerelease.");
...
export const verifyPublishedRelease = async ({
  ...
  const sbom = validateSbom(
    JSON.parse(await readFile(path.join(destination, expectedArtifacts[1]), "utf8")),
    version,
  );
