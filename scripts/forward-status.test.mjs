...
const statusArtifact = {
  ...
  packageId: "dynatrace-forward-example",
  mode: "apply",
  importState: "applied",
  packageSignature: {
    status: "verified",
...
test("treats reconciled Forward status as a ready import state", async () => {
  ...
  assert.equal(result.summary, "Forward synchronization status is ready for Dynatrace display.");
});
...
test("fetches a read-only status artifact URL", async () => {
  ...
  try {
    ...
    assert.equal(result.status, "ready");
    assert.equal(result.rows.find((row) => row.label === "Package")?.value, "dynatrace-forward-example");
  } finally {
