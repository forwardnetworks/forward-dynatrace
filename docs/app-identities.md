...
5. Create a package-handoff connection in the Dynatrace credential store. Never place secret values in Git or
   Workflow JSON.
6. Import the generated templates, select the new connection, and run an on-demand validation.
7. Verify package publication and aggregate status readback before enabling a schedule or problem trigger.
...
tenant URLs, dependency rows, hostnames, or path topology in this repository.
