...
> `docs/exec-plans/active/pre-1.0-product-readiness.md`; structural debt is tracked in
> `docs/exec-plans/tech-debt-tracker.md`.

This tracks what is implemented now and what remains before Forward for Dynatrace is generally available for
enterprise use. It is a pre-1.0 design-partner integration; support ownership, a signed preview, repeated acceptance,
and an explicit `1.0.0` promotion decision remain release gates.
...
- Done in repo: one strict `/v1` wire schema with no alternate-version or migration runtime. The schema identifier is
  independent of the `0.10.x` product version.
...
- External before wider use: provision signing keys if checksum-only integrity is not sufficient for the deployment
  trust model.
- Historical evidence only: `v1.0.0` through `v1.0.2` were built and independently verified, then retired when product
  maturity reset to `0.10.0`; they must not be installed.
- Done in repo: pin the package, ownership, import-plan, approval, and status contracts to the sole `/v1` wire schema.
...
- Done in repo: isolated mechanical tests publish a package, pull it, import it against fake Forward responses, and
  rerun the same package to confirm idempotency. These tests are never presented as live pilot or demo evidence.
