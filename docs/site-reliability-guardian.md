...
Dynatrace-owned service telemetry and confirms that passing, sanitized Forward evidence exists in the same window.
The Forward identity remains in an owner-controlled Dynatrace secret setting and is loaded only by the app function;
it never enters Guardian context, Grail evidence, the browser, endpoints, devices, or path topology.
...
| Network and before/after snapshot IDs | Dynatrace app backend through Forward APIs | Proves which modeled network evidence was evaluated. |
...
The commands below are repository acceptance tooling, not another customer runtime or release asset. The supported
product remains the Dynatrace app plus tenant-native Workflow and Guardian configuration. A shared non-production
pilot must execute the same event step as a reviewed Dynatrace Workflow action; do not schedule this CLI as a separate
Forward connector.
