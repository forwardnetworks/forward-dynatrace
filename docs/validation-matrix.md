...
| Release archive download verification | The `v0.9.9`, `v0.9.10`, `v0.9.11`, `v0.9.13`, and `v0.9.15` pre-1.0 GitHub release archives were downloaded and `SHA256SUMS` verified locally before the stable `v1.0.0` release reset. The `v1.0.0` release should be verified with the same checksum, signature, attestation, and archive-content checks after publication. |
...
| GHCR importer image workflow | `.github/workflows/release.yml` publishes `ghcr.io/forwardnetworks/forward-dynatrace-importer:<tag>` on tag releases and requests image/artifact attestations. The `v1.0.0` image must be inspected locally after publication and recorded in the release evidence. |
| Container vulnerability scan workflow | `.github/workflows/release.yml` scans the published GHCR importer image with `aquasecurity/trivy-action@v0.36.0`, fails on HIGH/CRITICAL findings, uploads SARIF through CodeQL, and publishes the SARIF as a workflow artifact. The `v1.0.0` Trivy SARIF artifact must be downloaded locally after publication and recorded in the release evidence. |
