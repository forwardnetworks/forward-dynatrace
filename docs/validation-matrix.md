...
| Runtime and release | `npm run runtime:validate`, `npm run systemd:install:test`, release signature/ref/immutability tests, `npm run security:audit`, and `npm run release:package:smoke` cover deployment templates, protected mounted authorization, exact release membership, and release provenance. | Provision the dedicated least-privilege runtime identity and customer platform ownership. |
