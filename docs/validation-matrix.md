...
| Release archive download verification | The `v1.0.0` release must be verified locally with `SHA256SUMS`, `SHA256SUMS.sig`, importer archive attestation, and GHCR image attestation. Earlier pre-1.0 archives were retained as `v0.9.x` prereleases. |
...
| GHCR importer image workflow | `.github/workflows/release.yml` publishes `ghcr.io/forwardnetworks/forward-dynatrace-importer:<tag>` on tag releases and requests image/artifact attestations. The published digest is recorded in the GitHub release notes after the release job completes. |
| Container vulnerability scan workflow | `.github/workflows/release.yml` scans the published GHCR importer image with `aquasecurity/trivy-action@v0.36.0`, fails on HIGH/CRITICAL findings, uploads SARIF through CodeQL, and publishes the SARIF as a workflow artifact. Download the release SARIF and confirm zero results as part of release evidence. |
