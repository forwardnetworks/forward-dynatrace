...
| Supported Node 24 release-candidate run | Full `npm ci && npm run ci` passed in a clean Linux/arm64 `node:24-alpine` container using Node `v24.18.0` on 2026-07-13. The polished portal-capture tree passed again in clean `node:24-bookworm` using Node `v24.18.0`, including all tests, the 2,500-row scale smoke, production dependency audit, app/action build, and release archive smoke. |
...
| Complete synthetic portal capture | `npm run demo:capture-evidence:test` builds non-empty Forward reconciliation, problem evidence, safe/regression ServiceNow gates, failure/recovery check-health, and security-correlation rows through the production builders, and verifies a single explicit synthetic provenance label across every row. `npm run demo:capture` disables live DQL only in capture mode, rejects visible live-query failures or missing headline evidence, and regenerates the checked screenshots from the built app. |
