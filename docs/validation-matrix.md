...
| Live Dynatrace demo dependency data | A read-only live query on 2026-07-13 returned 0 raw and 0 normalized dependency rows, so the conductor stopped before any Forward call. Customer-owned topology remains the production source of intent. The repo includes a saved 100-row standard demo fixture for approved replay into trial/demo sandboxes; replay remains visibly synthetic and is never automatic. |
