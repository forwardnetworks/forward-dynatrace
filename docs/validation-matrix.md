...
| Release archive download verification | The `v1.0.9`, `v1.0.10`, `v1.0.11`, `v1.0.13`, and `v1.0.15` GitHub release archives were downloaded and `SHA256SUMS` verified locally. The `v1.0.15` SBOM checksum and `SHA256SUMS.sig` were verified locally; app/importer archives were checked for governance docs, schemas, acceptance bundle scripts, status dashboard template, and status DQL files. |
...
| GHCR importer image workflow | `.github/workflows/release.yml` publishes `ghcr.io/forwardnetworks/forward-dynatrace-importer:<tag>` on tag releases and requests image/artifact attestations. The `v1.0.15` image was inspected locally at index digest `sha256:b2243c8cd17cc61da8d52e6843cb156023c49bdb878bbd0d58d5fe5d565f078b`. |
| Container vulnerability scan workflow | `.github/workflows/release.yml` scans the published GHCR importer image with `aquasecurity/trivy-action@v0.36.0`, uploads SARIF through CodeQL, and publishes the SARIF as a workflow artifact. The `v1.0.15` Trivy SARIF artifact was downloaded locally and contained 0 results. |
