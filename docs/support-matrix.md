...
| Forward network selection | Exact network ID is configured manually on each connection; v0.13.x does not discover networks from a Credential Vault entry or provide a dynamic network dropdown |
| Private/on-premises TLS | Certificate verification is mandatory. Use a trusted server chain or EdgeConnect with the private CA configured through `certificate_paths`; the app has no TLS-verification bypass |
...
| Distribution | Tenant-validated public GitHub release; signed Dynatrace distribution required for supported production |
...
certificate trust failure, or mismatched approval produces an operator-visible plan or error and does not broaden
access or mutate Forward.
