...
The native live-dependency view requires `storage:spans:read`. The manifest check is part of repository CI so a build
cannot silently ship the trace query without its matching read scope.
