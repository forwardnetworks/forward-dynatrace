...
Optional NQE execution requires a customer-approved authorization model. Read Only must use an approved Forward
Library query ID; Network Operator and Network Admin may use an approved arbitrary NQE template:
...
| --- | --- | --- |
| Connector access profile | Forward operator | Confirms the package and connector use the same Read Only, Network Operator, or Network Admin profile. Readiness forcibly disables all mutation flags before invoking the importer. |
...
| Optional NQE | Forward operator | Checks the optional Library-query or arbitrary-NQE preview path allowed by the selected profile. It is not required for package export/import. |
...
| NQE approval or authorization failure | Forward operator | Fix the profile, NQE approval, authorization header, query-ID allowlist, or Forward permission. |
