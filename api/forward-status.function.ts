...
export default async function (payload?: ForwardStatusRequest): Promise<ForwardStatusResponse> {
  ...
  return {
    status: "ready",
    summary: failed
      ? "Forward apply stopped and requires a new reconciliation plan."
      : needsReview
        ? "Forward synchronization completed with reviewable drift."
        : "Forward synchronization status is ready for Dynatrace display.",
    rows: [
    ...
    nextSteps: failed
      ? [
          ...
          "Retain the Workflow action result as the mutation audit record.",
        ]
      : needsReview
      ? [
          "Review changed and stale Dynatrace-managed checks in Forward.",
          "Keep update approval and stale-check review under explicit policy.",
          "Regenerate the package after Dynatrace mapping changes.",
        ]
      : [
          "Keep this as read-only Dynatrace evidence.",
          "Run the bundled Workflow action on the approved cadence.",
        ],
  };
}
