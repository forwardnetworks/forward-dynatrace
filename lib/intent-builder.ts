...
export type { ForwardSyncRequest } from "./types/index.ts";

export interface ForwardExportManifest {
...
export type ForwardIntentBuildResult = Omit<
  ForwardSyncResponse,
  "exportManifestPreview" | "intentChecksPreview"
> & {
  exportManifest: ForwardExportManifest | null;
...
export const buildForwardIntentPackage = (
  payload?: ForwardSyncRequest,
): ForwardIntentBuildResult => {
  ...
  const exportManifest = toExportManifest({
    payload,
    generatedAt,
    exportableDependencies,
    rejectedDependencyCount,
